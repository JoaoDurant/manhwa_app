...
def save_audio_tensor_to_file(
    ...
    try:
        ...
        # torchaudio nightly exige torchcodec (não instalado). Usar soundfile diretamente.
        import soundfile as sf
        import numpy as np
        audio_np = audio_tensor_cpu.squeeze(0).numpy()
        if audio_np.ndim > 1:
            audio_np = audio_np.T  # soundfile espera (samples, channels)
        sf.write(str(file_path), audio_np.astype(np.float32), sample_rate)
        end_time = time.time()
        logger.info(
