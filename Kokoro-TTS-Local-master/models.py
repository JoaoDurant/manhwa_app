...
def build_model(model_path: str, device: str, repo_version: str = "main", lang_code: str = 'a') -> EnhancedKPipeline:
    ...
    with _pipeline_lock:
        ...
        try:
            ...
            # Configuração das vozes IPA / Phonemizer
            espeak_map = {
                'a': 'en-us', 'b': 'en-gb', 'j': 'ja', 'z': 'zh',
                'e': 'es', 'f': 'fr-fr', 'h': 'hi', 'i': 'it', 'p': 'pt-br'
            }
            target_espeak_lang = espeak_map.get(lang_code, 'en-us')
            
            logger.info(f"Initializando phonemizer (IPA) para idioma mapeado: {target_espeak_lang}...")
            try:
                initialize_phonemizer(target_espeak_lang)
            except Exception as e:
                logger.warning(f"Could not initialize phonemizer for {target_espeak_lang}: {e}")
