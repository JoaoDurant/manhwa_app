...
def _smoothstep(t: float) -> float:
    """Classico Smoothstep Easing (Ease-in-out Perfeito)."""
    t = max(0.0, min(1.0, t))
...
def _python_render_clip(
    ...
    try:
        ...
        # Color correction leve para manhwa: contraste suave, saturação viva, sharpen nos traços
        # - eq=contrast=1.05:brightness=0.02:saturation=1.1 → evita imagem lavada
        # - unsharp=3:3:0.5:3:3:0 → sharpen leve nos contornos/texto sem artefatos
        ...
        # Aplica color correction leve no encode final
        cmd.extend(["-vf", COLOR_CORRECTION])
        cmd.extend([*vcodec, "-c:a", "aac", "-b:a", "192k", "-pix_fmt", "yuv420p", "-shortest", output_mp4])
        
        proc = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=subprocess.CREATE_NO_WINDOW if os.name == "nt" else 0)
        ...
        with torch.no_grad():
            for i in range(frames):
                ...
                # Equação exata do usuário:
                e = _smoothstep(t_linear)
                ...
                if is_split:
                    ...
                    anim_val = random.choice(["zoom_in", "zoom_out", "pan_up", "pan_down"]) if effect == "auto" else effect
...
class VideoPipeline(QObject):
    ...
    def run(self):
        ...
        # NVENC e AMF em placas modernas (RTX 40/50) suportam muitas sessões.
        # Maximizamos para usar todo o poder do processador e da GPU Blackwell (5070 Ti).
        if encoder != "libx264":
            max_workers = _CPU_COUNT * 2  # Concorrência Massiva para NVENC
        else:
            max_workers = _CPU_COUNT  # Todos os núcleos para libx264
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for idx, task in enumerate(clip_tasks):
                ...
                fut = executor.submit(
                    _python_render_clip,
                    image_path, audio_path, effect, duration, clip_mp4, FPS, encoder, None,
                    self.transition_mode, self.transition_time, idx
                )
        ...
        try:
            ...
            if result.returncode == 0 and out_path.exists():
                ...
                if getattr(self, "bg_music_path", ""):
                    ...
                    temp_mp4 = out_path.with_name(out_path.name + "_bgtemp.mp4")
                    ...
                    bgm_cmd = [
                        ...
                        "-stream_loop", "-1",
                        "-i", str(bg_path),
                        "-filter_complex", f"[1:a]volume={bg_vol}[bgm];[0:a][bgm]amix=inputs=2:duration=first:dropout_transition=2[aout]",
                        "-map", "0:v", "-map", "[aout]",
                    ...
                    if bg_res.returncode == 0 and temp_mp4.exists():
                        ...
                        err = bg_res.stderr.decode("utf-8", errors="replace")[-500:]
                        self.log_message.emit(f"  ⚠ Falha ao adicionar música de fundo:\n{err}\n(Mantendo vídeo sem música)")
